...
- FR-ONB-5: Society Admin uses one **Residents** screen on Client App Admin and Flutter Admin: **Directory** (searchable list) and **Pending invitations** (resend/revoke leftover tokens from CSV or older flows). **Add resident** opens the household onboard form (Owner / Family / Parking / vehicles / Gas). Optional Email / WhatsApp checkboxes send a **welcome** (sign in with OTP) — they do **not** create an invitation token. Staff create path is onboard-now, not a separate Invite form. Bulk CSV stays web-only. Move-out / verification detail remain on resident detail (web).
