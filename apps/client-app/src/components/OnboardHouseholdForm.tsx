...
export function OnboardHouseholdForm({
  showCsv = true,
  onSaved,
}: OnboardHouseholdFormProps) {
  ...
  function notifyChannels(): Array<"email" | "whatsapp"> | undefined {
  ...
  function deliveryMessage(
    ...
    if (delivery.email) {
      parts.push(
        delivery.email.ok ? "email sent" : `email failed: ${delivery.email.error}`,
      );
    }
    if (delivery.whatsapp) {
      parts.push(
        delivery.whatsapp.ok
          ? "WhatsApp queued"
          : `WhatsApp failed: ${delivery.whatsapp.error}`,
      );
    }
  ...
  useEffect(() => {
    ...
    client.listSocietyResidents().then(setResidents).catch(() => setResidents([]));
    client
      .listParkings()
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const editingOwner = isOwner && hasOwner;
    try {
      const res = await client.onboardResident({
        ...
        childCount: Number(childCount) || 0,
        seniorCitizenCount: Number(seniorCitizenCount) || 0,
        channels: editingOwner ? undefined : notifyChannels(),
      });
      setMessage(
        `Onboarded ${res.user.name} (${res.user.phone})${deliveryMessage(res.delivery)}`,
      );
      setName("");
      setPhone("");
      setEmail("");
  ...
  return (
    <div className="space-y-3" data-testid="onboard-household-form">
      {message && <p className="text-sm text-[var(--leaf)]">{message}</p>}
      {error && <p className="text-sm text-[var(--danger)]">{error}</p>}

      <form className="space-y-3" onSubmit={onSubmit} data-testid="onboard-form">
          ...
          {flatId && tab === "family" && (
            <>
              <p className="sh-span-2 text-xs text-black/50">
                Each family member needs their own mobile to log in and raise complaints.
              </p>
                ...
                onSave={async (draft) => {
                  ...
                  try {
                    const res = await client.onboardResident({
                      ...
                      childCount: Number(childCount) || 0,
                      seniorCitizenCount: Number(seniorCitizenCount) || 0,
                      channels: draft.userId ? undefined : notifyChannels(),
                    });
                    setMessage(
                      `${draft.userId ? "Updated" : "Added"} ${res.user.name} (${res.user.phone})${deliveryMessage(res.delivery)}`,
                    );
                    const people = await client.listSocietyResidents();
                    setResidents(people);
                    onSaved?.();
                  } catch (err) {
                    setFamilyError(
          ...
          {flatId && tab === "parking" && (
            <>
              <p className="sh-span-2 text-xs text-black/50">
                ...
                (Puzzle vs Open). Lots already on another flat stay listed but cannot be
                assigned here.
              </p>
        ...
        {flatId && (tab === "owner" || tab === "family") ? (
          <div
            className="flex flex-wrap gap-4 rounded-lg border border-[var(--sand)] bg-[var(--mist)]/30 px-3 py-2 text-sm"
            data-testid="onboard-notify"
          >
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                data-testid="onboard-notify-email"
                checked={notifyEmail}
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                data-testid="onboard-notify-whatsapp"
      ...
      {showCsv ? (
        <details className="card sh-section" data-testid="onboard-csv">
